<script lang="ts">
  ...
  async function saveTabEdit() {
    ...
    try {
      await api.put(`/api/categories/${editingTabId}`, { name: editingTabName.trim(), icon: editingTabIcon })
...
<div data-testid="dashboard-page">
  ...
  {:else}
    ...
    <div class="tab-bar mb-6 flex items-center gap-2">
      <div class="flex min-w-0 flex-1 flex-wrap items-center gap-2 overflow-x-auto">
        {#each $categories as cat, i (cat.id)}
          <div
            class="group relative flex shrink-0 items-center"
            role="listitem"
            draggable={$editMode && editingTabId !== cat.id}
            ondragstart={(e) => { if ($editMode) handleTabDragStart(e, i); else e.preventDefault() }}
            ondragover={(e) => { if ($editMode) handleTabDragOver(e, i) }}
            ondrop={(e) => { if ($editMode) handleTabDrop(e, i) }}
            ondragend={() => { if ($editMode) handleTabDragEnd() }}
          >
              <button
                onclick={() => handleTabClick(cat.id)}
                ondblclick={(e) => { if ($editMode) handleTabDblClick(e, cat.id, cat.name, cat.icon) }}
      ...
      {#if $editMode && selectedCategoryId !== null}
        <button
          onclick={() => { if (selectedCategoryId !== null) showAddWidgetMap[selectedCategoryId] = true }}
          class="shrink-0 rounded p-2 text-[var(--color-text-muted)] hover:text-[var(--color-primary)]"
    ...
    {#if showCreateInput}
      <button class="fixed inset-0 z-40" onclick={closeCreatePanel} aria-label="close"></button>
      <div class="fixed z-50 w-72 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-4 shadow-xl"
        style="top: {createPanelPos.top}px; left: {createPanelPos.left}px;">
        <div class="mb-3 flex items-center justify-between">
          <span class="text-sm font-semibold text-[var(--color-text)]">{$t('category.addCategory')}</span>
          <button onclick={closeCreatePanel} class="rounded p-1 text-[var(--color-text-muted)] hover:text-[var(--color-text)]">
            <X size={14} />
          </button>
        </div>
        <input
        ...
        />
        <div class="mb-3">
          <IconPicker selected={newCategoryIcon} onSelect={(icon) => newCategoryIcon = icon} />
        </div>
        <div class="flex justify-end gap-2">
          <button onclick={closeCreatePanel}
    ...
    {#if editingTabId !== null}
      <button class="fixed inset-0 z-40" onclick={closeEditPanel} aria-label="close"></button>
      <div class="fixed z-50 w-72 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-4 shadow-xl"
        style="top: {editPanelPos.top}px; left: {editPanelPos.left}px;">
        <div class="mb-3 flex items-center justify-between">
          <span class="text-sm font-semibold text-[var(--color-text)]">{$t('category.editCategory')}</span>
          <button onclick={() => { const id = editingTabId!; closeEditPanel(); handleTabDelete(id) }}
        ...
        />
        <div class="mb-3">
          <IconPicker selected={editingTabIcon} onSelect={(icon) => editingTabIcon = icon} />
        </div>
        <div class="flex justify-end gap-2">
          ...
          <button onclick={saveTabEdit} disabled={!editingTabName.trim()}
            class="rounded-lg bg-[var(--color-primary)] px-3 py-1.5 text-xs text-white hover:bg-[var(--color-primary-hover)] disabled:opacity-50">
            {$t('common.save')}
          </button>
        </div>
